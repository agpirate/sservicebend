...
const modelI = queModel
const modelIName = "/questions"
  ...
  try {
    return await modelI
      ...
      .then(async(modelQA) => { 
          ...
          //console.log('Creating New Questionaries',modelData,findBy)
...
let _officialMembers =['official','member']

let is_newCols='qcode'
router.post(modelIName, authenticate, async (req, res) => {
  ...
  if(!(userId && _prevAccess.includes(_issrole))){ 
    return res.status(401).send({ data: "Access Limited" });    
      ...
      //console.log(reqData,'Creating New Questionaries')
...
router.put(modelIName, authenticate, async (req, res) => {
  ...
  router.get(modelIName, authenticate,async (req, res) => {
    //-----------Authentications and Authorizations Params
    let [_issrole,userId,_userDep] = [req._issrole ?? false, req.userId ?? false,req._userDep ?? false]
    if(!userId){ 
      //return res.status(303).send({ message: "NullData(P) Received." });    
    }
    //--------------------
    ...
    //----------------
      if((_issrole != 'admin')){
        if((_issrole == 'ict')){
          findBy=[]
          findBy.push({ "creator_id":userId})
        }

        if(_issrole =='official'){
          findBy=[];findBy.push({ "schedule.end":{$lte: new Date() }}) 
        }
        else if(_issrole =='member'){
          findBy.push({ "schedule.end":{$lte: new Date() }}) 
        }
      }
    //--------------
    // findBy=[{}]
    try {
      let modelData = ''
      if(_prevAccess.includes(_issrole)){
         modelData = await rOrp(modelI, findBy, returnWat, limits);
      }else{
          modelData = await rOp(modelI, findBy, returnWat, limits);
      }
    //console.log(modelData,_issrole,findBy,'==========================')
...
//---------------------------Reading Filtered Data
const rOrp = async function (dbModel,findBy=[],returnWat={},limits=100) {
  try {
    return await dbModel
      ...
      .then((modelData) => {
        if (modelData && modelData.length) { return { status: 200, data: modelData }; //returns null or object_
        } else { return { status: 404, data:[] }; //not found Error
        } }).catch((modelError) => {
        return { status: 303, data: modelError }; //DBs_Schema or Rules_validations Error
      }); 
  } catch(error) { return { status: 303, data: error};  } //DBs Connections or Configurations, modules Error
